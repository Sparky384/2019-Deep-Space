...
/*
 * CAMERA NOTE The usb port at the top of the rio is device 1. Whichever camera
 * is plugged in here is device 1 (indexed at 0) and should be saved in the cam1
 * variable. As of now (3/13) the bottom camera should be cam1 (device index 0).
 * Once you plug in the cameras correctly and have tested them, do not unplug
 * them unless you HAVE to, this will lessen confusion as to which camera goes
 * where.
 * 
...
public class Robot extends IterativeRobot {
	...
	public void teleopInit() {
		...
		try (ServerSocket serverSocket = new ServerSocket(port)) {
			...
			while (!connected) {
				Socket socket = serverSocket.accept(); // TODO this blocks, that is not good
	...
	public void teleopPeriodic() {
		...
		driverControl(); // auton and teleop refer to one method to keep code clean and to avoid
							// confusion
		System.out.println("Xcentroid: " + ServerThread.xCentroid);
		System.out.println("Ycentroid: " + ServerThread.yCentroid);
	}
	...
	public void driverControl() {
		...
		// drive to cargo ship
		/*
		 * if (Input.getButtonPressed(Constants.PILOT, ButtonMap.toTarget)) targetTurn =
		 * true; if (Input.getButtonHeld(Constants.PILOT, ButtonMap.toTarget))
		 * toTarget(); if (Input.getButtonReleased(Constants.PILOT, ButtonMap.toTarget))
		 * targetTurn = false;
		 */

		// code to toggle cameras
		// for camera switching to work on the driver's station, the CameraServer Stream
		// Viewer
		// on the driver station has to have the property "Selected Camera Path" equal
		// to
		...
		if (Input.getButtonPressed(Constants.PILOT, ButtonMap.camera) && !cameraButtonLock) { // executes once when
																								// button pressed
		...
		if (Input.getButtonHeld(Constants.PILOT, ButtonMap.chaseBall)) {
			drive.cameraTurnTo(ServerThread.xCentroid, 5);
		}

		// Level 2 climb
		// *TODO* fix code so that the wristTimer is reset cleanly - right now we have
		// to
		...
		/*
		 * 
		 * // Why do we have 2 events - getButtonPressed and getButtonHeld? if
		 * (Input.getButtonPressed(Constants.COPILOT, ButtonMap.lvl2climb) && !auto) {
		 * if (!lvl2Lock) // level 2 climb initialization { wristSetpoint = 0;
		 * driveTimer.stop(); driveTimer.reset(); climberTimer.stop();
		 * climberTimer.reset(); state = Lvl2.IntakeDown; // initial start state
		 * lvl2Lock = true; // stays true until level 2 complete isWristMoving = false;
		 * elevating = false; // Climbing in level 2 only
		 ...
		 * boolean finished = false; int finished2 = 1; // The result from
		 * wrist.moveTo() - poor naming // Wrist initialization if (state ==
		 * Lvl2.IntakeDown && !isWristMoving) { wristSetpoint = Constants.WRIST_LVL2; //
		 * Establish setpoint, this is below the floor isWristMoving = true; // OK, the
		 * wrist is moving System.out.println("Entered intake down state, state = " +
		 * state); }
		 * 
		 * // Move wrist to low setpoint to raise robot front wheels if (isWristMoving
		 * && state == Lvl2.IntakeDown) { finished2 = wrist.moveTo(wristSetpoint, 2.5);
		 * // The result from wrist.moveTo() - poor naming
		 * System.out.println("Moving intake down to elevate robot = " + state); }
		 * 
		 * // Finish wrist low state, start drive state if (state == Lvl2.IntakeDown &&
		 * finished2 < 1) { isWristMoving = false; state = Lvl2.DriveToHab;
		 * System.out.println("Finish wrist low state, drive state, finish2 = " +
		 * finished2);
		 * System.out.println("Finish wrist low state, drive state, state = " + state);
		 * }
		 * 
		 * // In drive state if (state == Lvl2.DriveToHab) { int com =
		 * drive.driveTo(9.0, 1.0); System.out.println("Driving"); if (com < 1) { //
		 * Test for PID failure/complete state = Lvl2.StoreIntake; System.out.
		 * println("Driving completed/failed, start wrist store state, state = " +
		 * state); } }
		 * 
		 * // Start the wrist up to store position state if (state == Lvl2.StoreIntake
		 * && !isWristMoving) { isWristMoving = true;
		 * System.out.println("Entered wrist store (up) state, state = " + state); }
		 * 
		 * // Move the wrist up to store position if (state == Lvl2.StoreIntake &&
		 * isWristMoving) { finished = wrist.goToTop();
		 * System.out.println("Moving wrist to store posn, state = " + state); if
		 * (finished) { isWristMoving = false; state = Lvl2.Climb;
		 * System.out.println("Wrist store complete, start climber state, state = " +
		 * state); } }
		 * 
		 * // Start the climber state - this could all go in the if block above if
		 * (state == Lvl2.Climb && !elevating) { climberTimer.reset();
		 * climberTimer.start(); elevating = true; // Climbing in level 2 only }
		 * 
		 * // Climb if (elevating && state == Lvl2.Climb) climb.windClimb(); // This is
		 * the full speed climb with no accel ramp
		 * System.out.println("Climbing, state = " + state);
		 * 
		 * if (state == Lvl2.Climb && climberTimer.hasPeriodPassed(0.85)) { state =
		 * Lvl2.FinishDrive; driveTimer.reset(); driveTimer.start(); climb.stop();
		 * System.out.println("Entered finish state, state = " + state); } // Do the
		 * finish drive if (state == Lvl2.FinishDrive) { //climb.windClimb();
		 * drive.arcadeDrive(0.65, 0.25);
		 * System.out.println("Doing the finish drive, state = " + state); }
		 * 
		 * // Check for drive finished if (state == Lvl2.FinishDrive &&
		 * driveTimer.hasPeriodPassed(1.0)) { state = Lvl2.Stop;
		 * System.out.println("Entered stop state, state = " + state); }
		 * 
		 * // cleanup, level 2 routine finished if (state == Lvl2.Stop) { climb.stop();
		 * drive.arcadeDrive(0, 0); wrist.stop();
		 * System.out.println("Executing stop state, state = " + state); } }
		 * 
		 * if (Input.getButtonReleased(Constants.COPILOT, ButtonMap.lvl2climb)) {
		 * climb.stop(); drive.arcadeDrive(0, 0); wrist.stop(); wristSetpoint = 0;
		 * driveTimer.stop(); driveTimer.reset(); climberTimer.stop();
		 * climberTimer.reset(); lvl2Lock = false; isWristMoving = false; elevating =
		 * false; startClimberTimer = false; climberMaxSpeed = false; }
		 */
